...
if [ -z "${CUR_V}" ]; then
  ...
  DL_URL="$(wget -qO- https://api.github.com/repos/BeamMP/BeamMP-Server/releases | jq -r --arg LAT_V "$LAT_V" '.[] | select(.tag_name == $LAT_V) | .assets[] | .browser_download_url | match("^.*BeamMP-Server-linux*$") | .string' || jq -r --arg LAT_V "$LAT_V" '.[] | select(.tag_name == $LAT_V) | .assets[] | .browser_download_url | match("^.*BeamMP-Server-debian.*$") | .string')"
  cd ${DATA_DIR}
